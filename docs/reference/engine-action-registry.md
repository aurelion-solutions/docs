...
|---|---|
| `engine` | Non-empty identifier for the owning engine (e.g. `"provisioning"`) |
...
as of Phase 18 Step 21. The live catalogue (with arg/result schemas) is available
at `GET /api/v0/.well-known/pipeline-actions.json`.

Actions marked `idempotent=yes` are safe for the runner to retry. For
connector-backed actions (`provisioning.*`) this is a **delegated contract** —
...
|---|---|---|
| `effective_access.list_grants` | yes | Return a page of effective grants matching given filters |
| `effective_access.explain_access` | yes | Deny-wins aggregation for a (subject, resource, action) triple |
| `effective_access.get_grant` | yes | Fetch a single effective grant by id |
| `effective_access.project_access_fact` | no | Project all (fact, initiative) pairs for one AccessFact into effective_grants |
| `effective_access.project_application` | yes | Project all (fact, initiative) pairs for one Application into effective_grants |
| `effective_access.apply_incremental_change` | no | Apply one incremental inventory event to the effective_grants projection |
| `reconciliation.run` | yes | Run a full reconciliation cycle for one application |
| `reconciliation.master_data_apply` | yes | Apply reconciliation delta items for one entity type (persons, accounts, etc.) |
| `sync_apply.apply` | yes | Apply approved reconciliation delta items to normalized.access_facts |
...
| `provisioning.create_account` | yes | Provision a new account on a target application via connector |
| `provisioning.delete_account` | yes | Remove an account from a target application via connector |
