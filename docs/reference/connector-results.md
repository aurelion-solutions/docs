...
| `now` | ISO-8601 datetime (UTC) | Server-side ingest timestamp. Consumed by the `application_sync` pipeline as the projection cut-off (`access_effective.project_application` `now` arg). Always present. |
